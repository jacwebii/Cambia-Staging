<?php
	...
	function wbcr_hlp_get_plugin_options()
	{
		...
		if( !WHLP_Plugin::app()->getOption('hide_wp_admin') && !WHLP_Plugin::app()->getOption('hide_login_path') ) {
			$eventsOff = array(
		...
		$options[] = array(
			...
			'hint' => __("Hides the wp-login.php and wp-signup.php pages.", 'hide-login-page') . '<br>--<br><span class="hint-warnign-color">' . __("Use this option carefully! If you forget the new login page address, you can not get into the admin panel.", 'hide-login-page') . '</span>',
			'eventsOn' => array(
				'show' => '.factory-control-wpadmin_and_login_access_error',
			),
			'eventsOff' => $eventsOff
		...
		if( $plugin->getPluginName() == WHLP_Plugin::app()->getPluginName() && $is_form_page ) {
			...
			if( !empty($login_path) ) {
				...
				if( !$old_login_path || $login_path != $old_login_path ) {
					...
					$subject = sprintf(__('[%s] Your New WP Login!', 'hide-login-page'), WHLP_Plugin::app()
						->getPluginTitle());
