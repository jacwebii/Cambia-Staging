<?php
	...
	class WHLP_ConfigHideLoginPage extends Wbcr_FactoryClearfy200_Configurate {
		...
		{
			if( $this->getOption('wpadmin_and_login_access_error') == "redirect_to" ) {
				$access_redirect_to = $this->getOption('wpadmin_and_login_access_redirect', site_url() . '/404');
				wp_safe_redirect($access_redirect_to);
				die();
			} else if( $this->getOption('wpadmin_and_login_access_error') == "forbidden" && function_exists('status_header') ) {
				status_header('403');
				nocache_headers();
				die();
			} else {
